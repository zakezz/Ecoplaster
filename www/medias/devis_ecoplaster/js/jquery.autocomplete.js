	...
	$.Autocompleter = function(input, options) {
		...
		function selectCurrent() {
			...
			if (options.hiddenItem) {
				if ($(options.hiddenItemValue) != undefined) {
					$(options.hiddenItemValue).val(
							selected.data[1] + "|" + selected.data[2]);
				}
			}
			$('#waitAuto').html('');
			return true;
		...
		function request(term, success, failure) {
			if ($('#waitAuto') != undefined) {
				$('#waitAuto').html('<img src="ui/img/ajax-loader.gif"/>');
			}
			if (!options.matchCase)
			...
			if (data && data.length) {
				...
				var autoAjax = $.manageAjax.create('autoAjax', {
					queue : 'clear',
					cacheResponse : true,
